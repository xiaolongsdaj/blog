<template>
  <div class="tab-content">
    <div class="tab-header">
      <h2 class="tab-title">我的点赞</h2>
    </div>
    <div v-if="likedArticles.length > 0" class="article-list">
      <el-card
    ...
    <div v-else class="empty-state">
      <el-empty description="您还没有点赞任何文章" />
    </div>
    <el-pagination
      v-if="likedArticles.length > 0"
      layout="total, prev, pager, next"
      :total="likedArticlesTotal"
      :page-size="pageSize"
...
const likedArticlesTotal = ref(0)
...
const loadLikedArticles = async () => {
  loading.value = true
  try {
    const response = await articleApi.getUserLikedArticles(currentPage.value, pageSize.value)
    likedArticles.value = response.data.articles
    likedArticlesTotal.value = response.data.pagination.total
  } catch (error) {
    console.error('获取用户点赞文章列表失败:', error)
    ElMessage.error('获取点赞文章列表失败，请稍后重试')
  } finally {
    loading.value = false
  }
}
